
let obj = {
    all : {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            lgbtqia: [],
            asian: []
        },
        coffee: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }

    },
    Bates: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        woman: [],
        latino: [],
        black: [],
        immigrant: [],
        lgbtqia: [],
        asian: []
    },
    Linn: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    },
    Miami: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    },
    Johnson: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    },
    Wyandotte: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        woman: [],
        latino: [],
        black: [],
        immigrant: [],
        lgbtqia: [],
        asian: []
    },
    Leavenworth: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    },
    Platte: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    },
    Clay: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    },
    Clinton: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        woman: [],
        latino: [],
        black: [],
        immigrant: [],
        lgbtqia: [],
        asian: []
    },
    Caldwell: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    },
    Ray: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    },
    Lafayette: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    },
    Jackson: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        woman: [],
        latino: [],
        black: [],
        immigrant: [],
        lgbtqia: [],
        asian: []
    },
    Cass: {
        all: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        coffee:{
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        food: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        clothing: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        homegoods: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        },
        others: {
            all: [],
            woman: [],
            latino: [],
            black: [],
            immigrant: [],
            asian: [],
            lgbtqia: []
        }
    }
}





// Get array of all businesses
export const getAllBusinesses = async() => {
    try{

        const response = await fetch("http://localhost:8080/api/businesses");
        if(response.ok){
            const jsonResponse = response.json();
            return jsonResponse;
        }
        else {
            console.log("auth failed");
        }

    } catch(e){
        console.log(e);
    }
}



// Get business type into right section
const getRightBusinessType = (str) => {
    let arr = str.split(' ');
    let type = [];
    if(arr.includes('Food') || arr.includes('food')){
        type.push('food')
    }
    else if(arr.includes('Coffee') || arr.includes('coffee')){
        type.push('coffee')
    }
    else if(arr.includes('Clothing') || arr.includes('clothing')){
        type.push('clothing')
    }
    else if(arr.includes('Home') || arr.includes('home')){
        type.push('homegoods')
    }
    else {
        type.push('others')
    }
    return type[0]

}



// Add array into object
export const tranformIntoObj = (arr) => {
    arr.forEach(business => {
        business.businessType.name = getRightBusinessType(business.businessType.name)
        if(!obj.all.all.all.includes(business.id)){
            obj.all.all.all.push(business.id);
            obj[business.businessLocation.county].all.all.push(business.id);
            obj[business.businessLocation.county][business.businessType.name].all.push(business.id);
            obj.all[business.businessType.name].all.push(business.id);
            for(const owner of business.ownerType){
                obj[business.businessLocation.county].all[owner.name].push(business.id);
                obj.all.all[owner.name].push(business.id);
                obj.all[business.businessType.name][owner.name].push(business.id);
                obj[business.businessLocation.county][business.businessType.name][owner.name].push(business.id);
            }   
        }       

    })
    console.log(obj);
    return obj;
}







